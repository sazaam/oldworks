...
module.exports = Pkg.write('org.libspark.straw', function(){
	...
	var Address = Type.define({
		...
		statics:{
			...
			loc_re:/\/(a(f|m|rn?|s|z)|b(a|e|g|n|o|r|s)|c(a|o|s|y)|d(a|e|sb|v)|e(l|n|s|t|u)|f(a|il?|o|r|y)|g(a|d|l|sw|u)|h(a|e|i|r|sb|u|y)|i(d|g|i|s|t|u)|ja|k(a|k|l|m|n|ok?|y)|l(b|o|t|v)|m(i|k|l|n|oh|r|s|s|t)|n(b|e|l|n|so)|o(c|r)|p(a|l|rs|s|t)|qu(t|z)|r(m|o|u|w)|s(ah?|e|i|k|l|m(a|j|n|s)|q|r|v|w|yr)|t(a|e|g|h|k|n|r|t|zm)|u(g|k|r|z|z)|vi|wo|xh|yo|z(h|u))(?=\/|$)(.+)/
		},
		constructor:Address = function Address(str){
			var u = this ;
			u.absolute = str ;
			var loc_re = Address.loc_re ;
			...
			str.replace(Address.address_re, function(){
				...
				// all possible locales stands in that regexp
				// old regexp was this one
				// var loc_re = /\/([a-z]{2})(?=\/|$)/ ;
				
				if(loc_re.test(u.path))
					u.path = u.path.replace(loc_re, function(){
						u.loc = arguments[1] ;
						u.hash = arguments[arguments.length - 3]
						return u.hash[0] ;
					}) ;
	...
	var AddressHierarchy = Type.define({
		...
		sliceLocale:function sliceLocale(value){
			...
			path = value.replace(/^(a(f|m|rn?|s|z)|b(a|e|g|n|o|r|s)|c(a|o|s|y)|d(a|e|sb|v)|e(l|n|s|t|u)|f(a|il?|o|r|y)|g(a|d|l|sw|u)|h(a|e|i|r|sb|u|y)|i(d|g|i|s|t|u)|ja|k(a|k|l|m|n|ok?|y)|l(b|o|t|v)|m(i|k|l|n|oh|r|s|s|t)|n(b|e|l|n|so)|o(c|r)|p(a|l|rs|s|t)|qu(t|z)|r(m|o|u|w)|s(ah?|e|i|k|l|m(a|j|n|s)|q|r|v|w|yr)|t(a|e|g|h|k|n|r|t|zm)|u(g|k|r|z|z)|vi|wo|xh|yo|z(h|u))\//i, 
				function($0, $1){
					lang = $1 ;
					return '' ;
			}) ;
	...
	var AddressChanger = Type.define({
		...
		enable:function enable(loc, hierarchy, uniqueClass){
			...
			var useLocale = AddressHierarchy.parameters.useLocale ;
			...
			initLocale = useLocale ? document.documentElement.getAttribute('lang') : '',
			...
			if(!abshashReg.test(a.absolute)) { // means it never has been hashchanged, so need to reset hash...
				...
				var root = base ;
				// means we are trying to make the server know what is the root of project
				if(base !== a.base){
					
					a.path = a.absolute.replace(base, '') ;
					a.base = base ;
				...
				if(a.path === '/' && a.loc === '' && location.hash == '') {
					var p = useLocale ? '#' + separator + ch.locale + a.path + a.qs : '#' + a.path + a.qs ;
					location.hash = p ;
				}else{
					a.hash = '/' + a.path ;
					a.path = a.base ;
					
					loc.href = a.path + '#' + (useLocale ? separator + ch.locale : '') + a.hash + a.qs ;
				}
			}
			...
			$(window).bind('hashchange', function(e){ 
				trace('hashchange', location.hash) ;
				
				
				var address = a.base + a.path + location.hash ;
				var add = new Address(address) ;
				
				var locale = useLocale ? separator + add.loc : '' ;
				
				// if Locale is missing
				if(useLocale && add.loc == '') {
				   return ch.setValue(separator + ch.locale + add.hash + add.qs) ;
				}
				...
				if(AddressChanger.hasMultipleSeparators(add.hash)){
					return ch.setValue(locale + AddressChanger.removeMultipleSeparators(h)) ;
				}
				
				// if path is absent // hack for index VERY special case, we don't want to reload with same path ??!!
				if(add.hash == '/' && home != '') {
					return ch.setValue(locale + add.hash + (home == '' ?  home : home + separator) + add.qs) ;
				}
				
				// if last slash is missing
				if(!endSlashReg.test(add.hash)){
					// trace(locale + add.hash + separator + add.qs) ;
					ch.setValue(locale + add.hash + separator + add.qs) ;
				} else {
					ch.setValue(locale + add.hash + add.qs) ;
				}
