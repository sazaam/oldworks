...
package frocessing.text 
{
	...
	*/
	...
	{  
		...
		public var size:int;	//original size
		...
		/**
		 * 
		 * @param	ust_	vlw形式 を 読み込む URLStream
		 */
		public function PFont( ust_:IDataInput ) 
		{
			...
			_ascent  = ust.readInt();  // formerly baseHt (zero/ignored)
			_descent = ust.readInt();  // formerly ignored struct padding
			...
			{
				value[i]      = ust.readInt();
				height[i]     = ust.readInt();
				width[i]      = ust.readInt();
				setWidth[i]   = ust.readInt();
				topExtent[i]  = ust.readInt();
				leftExtent[i] = ust.readInt();
			...
			ust.readBytes( imgdat, 0, dat_index );
