...
package org.libspark.asreflect.builders
{
    ...
    public class BasicMethodHandler implements ElementHandler
    {
        ...
        {
            var declaringClass:Class = Class(getDefinitionByName(element.@declaredBy));
            var returnType:Class = null;
            if (element.@returnType == '*') {
                returnType = Object;
            }
            else if (element.@returnType != 'void') {
                returnType = Class(getDefinitionByName(element.@returnType));
            };
            var isStatic:Boolean = context.getValue('isStatic');
            context.push(new BasicMethod(element.@name, declaringClass, returnType, isStatic));
        }
