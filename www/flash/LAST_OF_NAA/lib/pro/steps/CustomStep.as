package pro.steps
{
	...
	import testing.DifferedCommand;
	...
	public class CustomStep extends VirtualSteps
	{
		...
		{
			var navStep:VirtualSteps = CustomNavStep(NavHierarchy.instance.getDeep(NavHierarchy.instance.functions.id + '/' + path.replace(ancestor.id + '/', ''))) ;

			var loader:XLoader = new XLoader() ;
			var url:String = '../xml/sections/' + xml.attribute('sections')[0].toXMLString() ;
		...
		private function resume(cond:Boolean = true):void 
		{
			var layer:Layer ;
			if (cond) {
				...
				//DifferedCommand(commandClose).dispatchComplete() ;
			}
		}
