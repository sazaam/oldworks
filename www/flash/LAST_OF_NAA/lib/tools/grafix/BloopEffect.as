package tools.grafix 
{
	...
	{
		private var __input:DisplayObject ;
		...
		public function BloopEffect(_input:DisplayObject, _segmentW:int, _segmentH:int = 0) 
		{
			...
			if (_input is Bitmap) {
				__params.input = __params.bmp = _input ;
			}else if(_input is DisplayObject){
				__params.input = _input ;
				__params.bmp = toBitmap(_input) ;
			}
			__params.x = _input.x ;
			__params.y = _input.y ;
			init() ;
		...
		private function toBitmap(s:DisplayObject):Bitmap
		{
		...
		public function show(x:Number, y:Number, closure:Function = null, delayMult:Number = -1):void
		{
			if(!__params.output.hasEventListener(Event.ENTER_FRAME)) __params.output.addEventListener(Event.ENTER_FRAME, onFrame) ;
			anim(x, y, false,closure, delayMult) ;
		}
		public function hide(x:Number, y:Number, closure:Function = null, delayMult:Number = -1):void
		{
			if(!__params.output.hasEventListener(Event.ENTER_FRAME)) __params.output.addEventListener(Event.ENTER_FRAME, onFrame) ;
			anim(x, y, true, closure, delayMult) ;
		}
		...
		public function toggle(x:Number, y:Number, closure:Function = null, delayMult:Number = -1):void
		{
			if (!__params.hidden) hide(x, y, closure, delayMult) else show(x, y, closure, delayMult) ;
		}
		private function onFrame(e:Event):void 
		{
			draw() ;
		}
		private function anim(x:Number, y:Number,cond:Boolean = true, f:Function = null, delayMult:Number = -1):void
		{
			...
			itw.onComplete = function():void {
				...
				if (Boolean(f)) f.apply(this) ;
			}
			itw.play() ;
		}
		
		public function appendTo(s:Sprite):Sprite
		{
			return Sprite(s.addChild(__params.output)) ;
		}
		public function removeFrom(s:Sprite):void
		{
		...
		public function set params(value:Object ):void { __params =  value }
		public function get input():DisplayObject { return __params.input }
		public function set input(value:DisplayObject):void { __params.input = value }
