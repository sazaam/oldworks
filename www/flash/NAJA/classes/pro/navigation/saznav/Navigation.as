package pro.navigation.saznav 
{
	...
	import naja.tools.func.Delay;
	import naja.tools.func.Delay;
	import naja.tools.steps.E.StepEvent;
	import naja.tools.steps.VirtualSteps;
	import pro.graphics.TestSamuae;
	import pro.navigation.saznav.highlight.Arrows ;
...
//////////////////////////////////////////////////////// GENERATE
		...
		public function readOnce(step:VirtualSteps):void
		{
			...
			if (!Boolean(step.userData.block as SubNavBlock)) {
				...
				.init(TestSamuae.item__tf, TestSamuae.small_item__tf)
				.read(step) ;
				addBlock(block) ;
			}else {
		...
		private function checkBlocksToUnfocus(block:SubNavBlock):void
		{
		...
		private function checkBlocksToFocus(block:SubNavBlock):void
		{
			var sNav:Smart = __nav.sub_nav ;
		...
		{
			var sNav:Smart = __nav.sub_nav ;
			var block:SubNavBlock = new SubNavBlock()
			.init(TestSamuae.item__tf, TestSamuae.small_item__tf)
			.read(step) ;
		...
		private function removeBlock(block:SubNavBlock):void
		{
			...
			Delay.make(1, null, unSetFocus, block, .1)
			.make(150, null, navExpand, .1)
			.make(150, null, backExpand, .15)
			.make(150, null, function():void {
				sNav.removeChild(block) ;
				checkBlocksToFocus(block) ;
			}) ;
		}
		private function addBlock(block:SubNavBlock):SubNavBlock
		{
			...
			var o:* = SubNavBlock(sNav.addChild(block)) ;
			return o ;
		}
		private function onBlockAdded(e:Event):void 
		{
			...
			checkBlocksToUnfocus(block) ;
			
			Delay.make(1, null, backExpand, .1)
			.make(150, null, navExpand, .15)
			.make(150, null, setFocus, block, .15)
			.make(1, null, checkBlocksToUnfocus, block)
			.make(150, null, ExternalDialoger.instance.hierarchy.dispatchReady) ;
		}
		private function setFocus(block:SubNavBlock , time:Number = 0):void
		{
			// if (!isNaN(__nav.arrowsTimeout)) clearTimeout(__nav.arrowsTimeout) ;
			// __nav.arrowsTimeout = setTimeout(checkBlockArrows, 0, block, time) ;
			// checkBlockArrows(block, time) ;
			// checkBlockArrows(block, time) ;
			block.focus(time) ;
		}
		private function checkBlockArrows(block:SubNavBlock , time:Number = 0):void
		{
			// var arrows:Arrows = __nav.arrows, logo:Sprite = __nav.logo_clickable ;
			// var rectArrows:Rectangle  ;
			// rectArrows = new Rectangle(block.x, block.y, block.width, block.properties.height) ;
			// rectArrows.offset(logo.width, 0) ;
			// arrows.posArrows(rectArrows, time) ;
		}
...
///////////////////////////////////////////////////////////////////////////////// NAV INITS
		...
		//////////////////////////////////////////////////////// ARROWS
		private function initArrows():Sprite
		{
			var logo:Sprite = __nav.logo_clickable ;
			var arrows:Arrows = __nav.arrows = new Arrows().init(logo.getRect(logo)) ;
			arrows.alpha = 0 ;
			__nav.logo.addChild(arrows) ;
			return arrows ;
		}
		//////////////////////////////////////////////////////// BACK
		...
		{
			var fshape:FShapeSVG = new FShapeSVG(TestSamuae.logo_svg);
