...
package frocessing.shape 
{
	...
	*/
	public class FShapeSVG extends FShapeContainer
	{
		 ...
		 */
		public function FShapeSVG( doc:XML, parent:FShapeContainer=null ) 
		{
			super( parent );
			if ( doc != null )
		 ...
		 * 
		 * TODO:http://www.w3.org/TR/SVG11/styling.html#UsingPresentationAttributes
		 */
		...
		public static function parseUse( doc:XML, parent:FShapeContainer = null, target:FShapeContainer=null ):FShapeContainer
		{
			...
			if ( obj is FShapeContainer )
			{
				...
				{
					target.addChild( cont.getChildAt(k) );
				}
				...
				if ( cont.viewbox != null )
				{
					if ( w!=0 && h != 0 )
					{
						target.matrix.prepend( cont.viewbox.getTransformMatrix(x, y, w, h) );
					}
					else if ( cont.viewbox.x != 0 && cont.viewbox.y != 0 )
					{
						target.matrix.prependTranslation( x, y );
						target.matrix.prependTranslation( -cont.viewbox.x, -cont.viewbox.y );
					}
				}
				else
				{
		...
		public static function parseImage( doc:XML, parent:FShapeContainer=null ):AbstractFShape
		{
			var x:Number  = parseLength( String(doc.@x), 0 );
			var y:Number  = parseLength( String(doc.@y), 0 );
			var w:Number  = parseLength( String(doc.@width), NaN );
			var h:Number  = parseLength( String(doc.@height), NaN );
			var href:String = (_xlink != null) ? doc.@_xlink::href : doc.@href;
		...
		private static function __initGradientModule( doc:XML, kind:String, mat:FGradientMatrix ):FShapeGradient
		{
			...
			var grad:FShapeGradient = new FShapeGradient( kind, colors, alphas, ratios, mat, spreadMethod, interpolationMethod );
			grad.isNormal = _bounding;
		...
		private static function parseStylesAttrib( doc:XML, target:AbstractFShape ):void
		{
			...
			if ( val != "" )
				target.alpha = opacity(val);
			
			val = String(doc.@["fill-opacity"]);
			if ( val != "" )
				target.fillAlpha = opacity(val);
			
			val = String(doc.@["stroke-opacity"]);
			if ( val != "" )
				target.strokeAlpha = opacity(val);
				
			val = String(doc.@["stroke-width"]);
			if ( val != "" ) 
				target.thickness = strokeWeight(val);
			
			val = String(doc.@["stroke-linejoin"]);
			if ( val != "" )
				target.joints = strokeJoin(val);
			
			val = String(doc.@["stroke-linecap"]);
			if ( val != "" )
				target.caps = strokeCap(val);
				
			val = String(doc.@["stroke-miterlimit"]);
			if ( val != "" )
				target.miterLimit = strokeMiterLimit(val);
				...
				for ( var i:int = 0; i < styleNum; i++ )
				{
					...
					switch( sname )
					{
						...
						case "stroke":
							...
							break;
						case "fill-opacity":
							target.fillAlpha = opacity(tokens[1]);
							break;
						case "stroke-opacity":
							target.strokeAlpha = opacity(tokens[1]);
							break;
						case "stroke-width":
							target.thickness = strokeWeight(tokens[1]);
							break;
						case "stroke-linecap":
							target.caps = strokeCap(tokens[1]);
							break;
						case "stroke-linejoin":
							target.joints = strokeJoin(tokens[1]);
							break;
						case "stroke-miterlimit":
							target.miterLimit = strokeMiterLimit(tokens[1]);
							break;
						case "opacity":
							target.alpha = opacity(tokens[1]);
							break;
						default:
		...
		private static function setStroke( strokeText:String, target:AbstractFShape ):void
		{
			...
			else if ( (url = parseUrl(strokeText)) != "" ) 
			{
				...
				if ( dat is FShapeGradient )
				{
					...
					target.strokeGradient = FShapeGradient( dat ); //opacity
				}
				else
				{
		...
		private static function setFill( fillText:String, target:AbstractFShape ):void
		{
			...
			else if ( (url = parseUrl(fillText)) != "" ) 
			{
				...
				if ( dat is FShapeGradient )
				{
					...
					target.fillGradient = FShapeGradient( dat ); //opacity
				}
				else
				{
					_err("url " + url + " refers to unexpected data");
		...
		public static function parseTransform( attr:String ):FMatrix2D
		{
			...
			var trans:Array = attr.match(/\w+\(.*?\)/g); //TODO "name" wsp* "("
