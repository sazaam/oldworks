...
public class as3Query extends Proxy {
	...
	static public function create(... args:Array):as3Query {
	...
	public function attr(key:Object, value:Object = null, type:String = null):Object {
		...
		return each(function(index:int, val:Object):void {
			...
			for ( var prop:String in obj ) {
				if (key is XML) {
					for each(var a:XML in XML(key).attributes()) {
						as3Query[type || "attr"](
							this,
							a.localName(), as3Query.prop(this, a.toXMLString(), type, index, prop)
						)
					}
					break ;
				}
					...
					try {
						...
						if ( ret ) {
							...
							for each ( var attr:XML in xml.attributes() ) {
								if (attr.toString() == "false") {
									obj[attr.localName()] = false ;
								}else if (attr.toString() == "true") {
									obj[attr.localName()] = true ;
								}else {
							...
							for each ( var node:XML in xml.*) {
								...
								if (ret.hasOwnProperty(node.localName())) {
									
									//FILTERS
									if (node.localName() == "filters") {
										var filters:Array = [] ;
										for each(var filter:XML in node.*) {
											var F:Class = getDefinitionByName( filter.localName() ) as Class ;
											var f = new F() ;
											var filterObj:Object = { };
											for each ( var a:XML in filter.attributes() ) {
												if (a.toString() == "false") {
													filterObj[a.localName()] = false ;
												}else if (a.toString() == "true") {
													filterObj[a.localName()] = true ;
												}else {
													//
													filterObj[a.localName()] = a.toString();
												}
											}
											$(f).attr( filterObj );
											filters.push(BitmapFilter(f)) ;
										}
										ret.filters = filters ;
									}
									
									try {
										ret[node.localName()] = node.toString();
									}
									catch (e:Error)
									{
										//trace(".....................later") ;
									...
									try {
										var T:Class = getDefinitionByName( node.*[0].localName() ) as Class;
										var resultObj:* = new T() ;
										ret[node.localName()] = resultObj ;
										
										var o:Object = {};
										for each ( var at:XML in node.*[0].attributes() ) {
											if (at.toString() == "false") {
												o[at.localName()] = false ;
											}else if (at.toString() == "true") {
												o[at.localName()] = true ;
											}else {
												o[at.localName()] = at.toString();
											}
											//trace(at.localName(),'  ',o[at.localName()])
										}
										$(resultObj).attr( o );
										if (resultObj is TextFormat) {
											if (Boolean(node.parent().attribute('embedFonts')[0]) && node.parent().attribute('embedFonts')[0].toXMLString() == 'true') {
												TextField(ret).embedFonts = true ;
											}
											TextField(ret).defaultTextFormat = resultObj ;
											TextField(ret).setTextFormat(TextField(ret).defaultTextFormat);
										}
									}
									catch (e:Error)
									{
