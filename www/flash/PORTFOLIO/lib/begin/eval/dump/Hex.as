...
package begin.eval.dump {
	...
	public class Hex {
		...
		public static function fromArray(array : ByteArray, colons : Boolean = false) : String {
			...
			var f : Function;
			for (var i : uint = 0;i < array.length;i++) {
				f = array[i]["toString"] as Function;
				s += ("0" + f(16)).substr(-2, 2);
		...
		public static function undump(str : String) : ByteArray {
			...
			var len : int = int(lines[0]["length"]);
			var stuff : Array;
			var val : String;
			var nlen : int;
			while (lines.length > 0 && len > 0) {
				val = String(lines.shift()).substr(10).split("  |")[0];
				stuff = val.split(/\s+/);
				nlen = int(stuff[0]["length"]);
				while (stuff.length > 0 && nlen > 0)
