package of.app.required.steps 
{
	...
	public class VirtualSteps extends StepList implements IStep
	{
		...
		override public function add(step:IStep):IStep
		{
			...
			step.index = __history.indexOf(step) ;
			register(step) ;
		...
		override public function remove(_id:Object = null):IStep
		{
			...
			item.index = -1 ;
			item.depth = 0 ;
			var n:int = __history.indexOf(item) ;
			__history = __history.slice(0, n).concat(__history.slice(n + 1, __history.length));
			//if (__playhead == n) 
			__playhead -- ;
			item.parent = null ;
			return item ;
		...
		{
			var p:Array = [].concat(__gates.merged) ;
			var l:int = p.length ;
...
///////////////////////////////////////////////////////////////////////////LAUNCH
		...
		{
			//to do check if inserting a 'cancel' here doesn't mess around too much 
			if (__isCancellable == true ) {
				__isCancellable = false ;
			}
			//if (history.length == 0) trace("No such int callable Step in HistoryList... " + __id) ;
			if (_num == -1) return ;
			
			if (__currentStep && __currentStep.id != _num && __currentStep.opened) __currentStep.stop() ;
			if (_num is String) __currentStep = __gates[_num] ;
			else if(_num is IStep) __currentStep = IStep(_num) ;
			else __currentStep = __gates.merged[_num] ;
			
			if (!Boolean(__currentStep)) throw new VerifyError('no step inside VStep, cannot launch...', this) ;
...
///////////////////////////////////////////////////////////////////////////GETTERS & SETTERS
		...
		public function destroy():IStep
		{
			if (id in __hierarchies) {
				__hierarchies[id] = null ;
				delete __hierarchies[id] ;
			}
			__userData = destroyUserData() ;
			__history = destroyHistory() ;
			__gates = destroyGates() ;
			return null ;
		}
		
		private function destroyHistory():Array 
		{
			removeAll() ;
			return null ;
		}
		
		private function destroyGates():Gates
		{
			gates.destroy() ;
			return null ;
		}
		private function destroyUserData():Object 
		{
			for (var p:String in __userData) {
				__userData[p] = null ;
			}
			return null ;
		}
		...
		public function get history():Array { return __history }
	}
}