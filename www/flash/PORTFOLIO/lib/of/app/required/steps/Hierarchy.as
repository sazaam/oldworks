...
//////////////////////////////////////////////////////////////////////////////////////////////// CORE FUNC
		...
		private function openCommand(path:String, command:DifferedCommand):void 
		{
			...
			if (!Boolean(st as VirtualSteps)) {
				var he:HierarchyError = new HierarchyError('commandQueue cannot execute >> ' + this + '\n>>> ' + "step doesn't exist", 9999) ;
				__changer.currentPath = path.replace(/\/*[^\/]+\/*$/, '') ;
				HierarchyError(he).errorPath = __changer.futurePath ;
				HierarchyError(he).currentPath = __changer.currentPath ;
				var e:HierarchyErrorEvent = new HierarchyErrorEvent(HierarchyErrorEvent.PATH_ERROR) ;
				HierarchyErrorEvent(e).error = he ;
				e.errorPath = he.errorPath ;
				e.currentPath = he.currentPath ;
				
				if (__changer.hasEventListener(HierarchyErrorEvent.PATH_ERROR)) __changer.dispatchEvent(e) ;
				
				return __commandQueue.reset() ;
			}
