package of.app.required.loading
{
	...
	public class XLoaderRequest extends EventDispatcher
	{
		...
		//////////////////////////////////////////////////////// CTOR
		public function XLoaderRequest(url:String, id:String, fileFormat:String = null, cacheEnabled:Boolean = true):void
		{
			this.url = url ;
			this.id = id ;
			var arr:Array = url.match(BasicRegExp.ext_STRICT_RE) ;
			this.fileFormat = fileFormat || (!Boolean(arr as Array))? 'swf' :  arr[0] ;
			this.ext = LoadingsController.testUrl(url, false) ;
		...
		internal function loadBytes(__data:Object):void
		{
		...
		{
			if (loader is Loader) {
				if(Loader(loader).hasEventListener(Event.COMPLETE)) Loader(loader).close() ;
				Loader(loader).unload() ;
				if(response && response is Sprite) Loader(loader).unloadAndStop() ;
			}
			id = null ;
			fileFormat = null ;
			cacheEnabled = true ;
...
///////////////////////////////////////////////////////////////////////////////// GETTERS & SETTERS
		...
		{
			if (!(value is Loader || value is URLLoader || value == null)) throw new Error("Loader must be either a Loader or an URLLoader, or null...", this) ;
