package of.app.required.loading
{
	...
	public class XLoader extends EventDispatcher
	{
		...
		public function removeRequest(request:XLoaderRequest):XLoaderRequest
		{
		...
		public function removeRequestById(id:String):XLoaderRequest
		{
			return removeRequest(getRequestById(id)) ;
		}
		public function removeRequestByUrl(url:String):XLoaderRequest
		{
			return removeRequest(getRequestByUrl(url)) ;
		}
		public function removeAllRequests():Object
		{
			for each (var request:XLoaderRequest in __reqs)
				request = removeRequest(request) ;
			return null ;
		}
		public function destroy():XLoader 
		{
			__reqs = __requests = removeAllRequests() ;
			return null ;
		}
		///////////////////////////////////////////////////////////////////////////////// GET REQUEST
		...
		public function getResponseById(id:String):*
		{
			for each (var req:XLoaderRequest in __reqs) {
				if (id == req.id) {
					...
					else throw new ArgumentError("Request with such ID has no response... yet... >> id : "+ id) ;
				}
			}
			throw new ArgumentError("Response wasn't found - Wrong ID >> id : "+ id);
		}
		...
		public function loadBytes(data:Object):void
		{
			for each (var request:XLoaderRequest in __reqs) {
				addEventListenerTo(request) ;
