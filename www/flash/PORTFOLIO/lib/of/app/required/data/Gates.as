package of.app.required.data
{
	...
	dynamic public class Gates extends Dictionary
	{
		...
		{
			__numeric = destroyArray(__numeric) ;
			__keys = destroyArray(__keys) ;
			__references = destroyRef(__references) ;
		}
		
		private function destroyArray(arr:Array):Array 
		{
		...
		private function destroyRef(d:Dictionary):Dictionary 
		{
			for(var i:* in d) {
				delete d[i] ;
			}
