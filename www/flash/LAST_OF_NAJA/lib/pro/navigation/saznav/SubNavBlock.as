package pro.navigation.saznav
{
	...
	public class SubNavBlock extends Smart
	{
		...
		private function onNavLinkClicked(e:MouseEvent):void
		{
			...
			XExternalDialoger.instance.swfAddress.value = str ;
		}
		public function read(step:VirtualSteps):SubNavBlock
		{
			...
			if (properties.adress == String(step.ancestor.id)) properties.adress == XExternalDialoger.instance.swfAddress.home ;
			buildSubNav() ;
			var w:int = int(width + properties.margin * 2) ;
			var h:int = int(height + properties.margin * 2) ;
			var back:Sprite = properties.back ;
			Draw.redraw("rect", { g:graphics, color:0xFFFFFF, alpha:.2 }, 0, 0, w, h) ;
			Draw.redraw("rect", { g:back.graphics, color:0xFFFFFF, alpha:.23 }, 0, 0, w, h) ;
			back.alpha = 0 ;
