...
<div class="x_control-group">
	...
	<div class="x_controls">
		<select name="skin" id="skin">
			...
			<?php if($__Context->skin_list&&count($__Context->skin_list))foreach($__Context->skin_list as $__Context->skin_name=>$__Context->skin){ ?><option value="<?php echo $__Context->skin_name ?>"><?php echo $__Context->skin->title ?>(<?php echo $__Context->skin_name ?>)</option><?php } ?>
		</select>
	...
	<div class="x_control-group <?php if($__Context->var->type == 'mid' || $__Context->var->type == 'module_srl_list'){ ?>moduleSearch moduleSearch1 modulefinder<?php } ?>">
		...
		<div class="x_controls">
			...
			<?php if($__Context->var->type == 'color'){ ?>
				<input type="text" name="<?php echo $__Context->id ?>" value="" id="<?php echo $__Context->id ?>" class="color-indicator" style="width:178px" />
			<?php } ?>
...
if($__Context->var->init_options[$__Context->key]){ ?><option value="<?php echo $__Context->key ?>"<?php if($__Context->var->default_options[$__Context->key]){ ?> default="true"<?php } ?>><?php echo $__Context->val ?></option><?php }} ?>
			...
			<?php if($__Context->var->type == 'module_srl_list'){ ?>
				...
				<br>
				<a href="#" id="__module_srl_list_<?php echo $__Context->id ?>" class="x_btn moduleTrigger" data-multiple="true" style="margin:0 -5px 0 0;border-radius:2px 0 0 2px"><?php echo $__Context->lang->cmd_add ?></a>
			...
			<?php if($__Context->var->type == 'menu'){ ?>
				<select name="<?php echo $__Context->id ?>">
					<option value="">-</option>
					<?php if($__Context->menu_list&&count($__Context->menu_list))foreach($__Context->menu_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->val->menu_srl ?>"><?php echo $__Context->val->title ?></option><?php } ?>
				</select>
			<?php } ?>
