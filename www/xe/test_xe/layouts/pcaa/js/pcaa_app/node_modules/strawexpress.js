...
module.exports = Pkg.write('org.libspark.straw', function(){
	...
	var Proxy = Type.define(function(){
		var ns = {}, __global__ = Type.appdomain, returnValue = function(val, name){return val },
		...
		DOMClass = function (obj) {
			...
			if(!!!tname) {
				if(obj === Type.appdomain){
					tname = 'WINDOW' ;
				}else
				
				for(var s in Type.appdomain){
					if(obj == Type.appdomain[s]) {
						tname = s.toUpperCase() ;
						break ;
					}
				}
	...
	var Express = Type.define({
		...
		statics:{
			app:undefined,
			disp:new EventDispatcher(Type.appdomain),
			initialize:function(){
		...
		initJSAddress:function initJSAddress(){
			var tg = Express.app.get('unique').getInstance() ;
			tg.commandOpen.dispatchComplete() ;
			
			return Express.app ;
		},
		get:function get(pattern, handler, p){
			
			
			if(arguments.length == 1){ // is a getter
				return Express.app.set(pattern) ;
			}
			
			if(typeof handler != 'function'){
				
				for(var s in handler)
					Express.app.get(s == 'index' ? '/' : s , handler[s], p) ;
				return Express.app ;
			}
			...
			res.handler = handler ;
			Express.app.enableResponse(true, res, p) ;
			return Express.app ;
		},
		set: function set(setting, val){
			if (1 == arguments.length) {
				if (Express.app.settings.hasOwnProperty(setting)) {
					return Express.app.settings[setting] ;
				} else if(!!Express.app.parent) {
					return Express.app.parent.set(setting) ;
				}
			} else {
				Express.app.settings[setting] = val ;
				return Express.app ;
			}
		},
		enableResponse:function enableResponse(cond, res, p){
			var handler = res.handler ;
			
			
			if(cond){
				
				p = p || AddressHierarchy.hierarchy.currentStep ;
				
				
				if(res.id == '') p.defaultStep = res ;
				p.add(res) ;
				
				for(var s in handler){
					if(s.indexOf('@') == 0) Express.app.attachHandler(true, s, handler[s], res) ;
					else if(s == 'index') Express.app.get('', handler[s], res) ;
					else Express.app.get(s, handler[s], res) ;
				}
				
			}else{
				
				for(var s in handler){
					if(s.indexOf('@') == 0) Express.app.attachHandler(false, s, handler[s], res) ;
				}
				var l = res.getLength();
				while(l--){
					// trace(res.getChild(l))
					Express.app.enableResponse(false, res.getChild(l), res) ;
				}
