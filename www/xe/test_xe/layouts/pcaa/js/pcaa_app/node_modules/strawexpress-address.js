	...
	var AddressChanger = Type.define({
		...
		statics:{
			hashEnable:function hashEnable(href){
				return '#' + href.replace(new RegExp(Type.appdomain.location.protocol + '//' + Type.appdomain.location.host), '').replace(/\/*$/,'/').replace(/^\/*/,'/').replace(/\/\/+/, '/') ;
			},
		...
		enable:function enable(loc, hierarchy, uniqueClass){
			...
			$(Type.appdomain).bind('hashchange', function(e){ 
				...
				setTimeout(function(){
					if(!weretested){
						...
						if(str == '/' && ch.getValue() == '' && AddressHierarchy.parameters.home == '' && Unique.getInstance().getChild('') === undefined)
							...
							$(Type.appdomain).trigger('hashchange') ;
					}
				}, 0) ;
			}) ;
