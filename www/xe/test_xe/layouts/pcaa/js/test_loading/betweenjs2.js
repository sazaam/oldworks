...
module.exports = Pkg.write('org.libspark.betweenjs', function(path){
	...
	var CSSPropertyMapper = Type.define({
		...
		statics:{
			...
			cssColorGet:function(target, pname, unit){
				...
				if(BetweenJS2.colormode == 'hsv'){ // only h, s, v in o
			...
			cssColorSet:function(target, pname, unit, val){
			    ...
			    if (BetweenJS2.colormode == 'hsv' && 'h' in val && 's' in val && 'v' in val){
	...
	var UpdaterFactory = Type.define({
		...
		checkStringObj:function(val, n, res){
			...
			if(BetweenJS2.colormode == 'hsv'){return CSSPropertyMapper.RGBtoHSV(res)}
			return res ;
		}
	});
	// SINGLE.TICKER
	...
	var BetweenJS2 = Type.define({
		domain:Type.appdomain,
		constructor:BetweenJS2 = function BetweenJS2(){
			throw 'Not meant to be instanciated... BetweenJS2::ctor' ;
		},
		statics:{
			...
			/*
				...
				(also set to tick forever from start, to disable, @see BetweenJS2.ticker.stop())
			*/
			initialize:function initialize(domain){
				var exclude = {
					'getTimer':undefined,
				...
				for(var n in BetweenJS2){
					(function(ind){
						if(typeof(BetweenJS2[ind]) == 'function' && !(ind in exclude)){
							var ff = BetweenJS2[ind] ;
							 
							BetweenJS2[ind] = function(target){
								var tar , arr ;
								...
								if('jquery' in target) { // is jquery element
									...
									if(s > 1){
										...
										return BetweenJS2.parallelTweens(arr) ;
									}else if(s == 1){
										tar = args.shift() ;
			...
			*/
			tween:function tween(target, to, from, time, easing){
				var tween = new ObjectTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.create(target, to, from) ;
				tween.time = time || 1.0 ;
			...
			*/
			to:function to(target, to, time, easing){
				var tween = new ObjectTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.create(target, to, undefined) ;
				tween.time = time || 1.0 ;
				tween.easing = easing || Linear.easeNone ;
				return tween ;
			},
			...
			*/
			from:function from(target, from, time, easing){
				var tween = new ObjectTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.create(target, undefined, from) ;
				tween.time = time || 1.0 ;
				tween.easing = easing || Linear.easeNone ;
				return tween ;
			},
			...
			*/
			bezier:function bezier(target, to, from, controlPoint, time, easing){
				var tween = new ObjectTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createBezier(target, to, from, controlPoint) ;
				tween.time = time || 1.0 ;
				tween.easing = easing || Linear.easeNone ;
				return tween ;
			},
			...
			*/
			bezierTo:function bezierTo(target, to, controlPoint, time, easing){
				var tween = new ObjectTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createBezier(target, to, undefined, controlPoint) ;
				tween.time = time || 1.0 ;
				tween.easing = easing || Linear.easeNone ;
				return tween ;
			},
			...
			*/
			bezierFrom:function bezierFrom(target, from, controlPoint, time, easing){
				var tween = new ObjectTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createBezier(target, undefined, from, controlPoint) ;
				tween.time = time || 1.0 ;
				tween.easing = easing || Linear.easeNone ;
				return tween ;
			},
			...
			*/
			physical:function physical(target, to, from, easing){
				var tween = new PhysicalTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createPhysical(target, to, from, easing || Physical.exponential()) ;
				return tween ;
			},
			/*
				physicalTo
			...
			*/
			physicalTo:function physicalTo(target, to, easing){
				var tween = new PhysicalTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createPhysical(target, to, undefined, easing || Physical.exponential()) ;
				return tween ;
			},
			/*
				physicalFrom
			...
			*/
			physicalFrom:function physicalFrom(target, from, easing){
				var tween = new PhysicalTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createPhysical(target, undefined, from, easing || Physical.exponential()) ;
				return tween ;
			},
			/*
				physicalApply
			...
			*/
			physicalApply:function physicalApply(target, to, from, applyTime, easing){
				if(applyTime === undefined) applyTime = 1.0 ;
				var tween = new PhysicalTween(BetweenJS2.ticker) ;
				tween.updater = BetweenJS2.updaterFactory.createPhysical(target, to, from, easing || Physical.uniform()) ;
				tween.update(applyTime) ;
				return tween ;
			},
			/*
			...
			*/
			parallel:function parallel(tween){
				return BetweenJS2.parallelTweens([].slice.call(arguments)) ;
			},
			/*
				parallelTweens
			...
			*/
			parallelTweens:function parallelTweens(tweens){
				return new ParallelTween(tweens, BetweenJS2.ticker, 0) ;
			},
			/*
				serial
			...
			*/
			serial:function serial(tween){
				return BetweenJS2.serialTweens([].slice.call(arguments)) ;
			},
			/*
				serialTweens
			...
			*/
			serialTweens:function serialTweens(tweens){
				return new SerialTween(tweens, BetweenJS2.ticker, 0) ;
			},
			/*
				reverse
			...
			*/
			addChild:function addChild(target, parent){
				return new AddChildAction(BetweenJS2.ticker, target, parent) ;
			},
			/*
				removeFromParent
			...
			*/
			removeFromParent:function removeFromParent(target){
				return new RemoveFromParentAction(BetweenJS2.ticker, target) ;
			},
			/*
				func
			...
			*/
			func:function func(ffunc, params, useRollback, rollbackFunc, rollbackParams){
				return new FunctionAction(BetweenJS2.ticker, ffunc, params, useRollback, rollbackFunc, rollbackParams) ;
			},
			/*
				timeout
			...
			*/
			timeout:function timeout(duration, func, params){
				var uid = getTimer() ;
				var tw = new TimeoutAction(BetweenJS2.ticker, duration, func, params) ;
				tw.uid = uid ;
			...
			*/
			interval:function interval(timer, func, params){
				var uid = getTimer() ;
				var tw = new IntervalAction(BetweenJS2.ticker, timer, func, params) ;
				tw.uid = uid ;
