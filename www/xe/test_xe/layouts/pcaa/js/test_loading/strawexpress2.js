...
module.exports = Pkg.write('org.libspark.straw', function(){
	...
	var Request = Type.define({
		...
		constructor:Request = function Request(url, complete, options) {
			var r = Request.generateXHR() ;
			if (!r) return ;
			var merge = function(o, o2){ for(var s in o) if(!! o2[s]) o[s] = o2[s] }
			this.request = r ;
			this.url = url ;
			this.complete = complete ;
			var hasData = (!! options && !!options.post_data) ;
			var p = {
				post_data: hasData ? options.post_data : undefined,
				post_method: hasData ? "POST" : "GET",
				ua_header: {ua:'User-Agent',ns:'XMLHTTP/1.0'},
				post_data_header: hasData ? {content_type:'Content-type',ns:'application/x-www-form-urlencoded'} : undefined 
			}
			if(!! options) merge(p, options) ;
			this.userData = p ;
		},
		load:function(url){
			var r = this.request ;
	...
	var Response = Type.define({
		...
		renderHTML:function(url, selector){
			var res = this ;
			var t = new Jade(url).load(undefined, false) ;
			
			res.template = $(t.response).children().find(selector) ;
