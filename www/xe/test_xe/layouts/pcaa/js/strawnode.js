...
(function(included){
	...
	var checkBase = function checkBase(){
		...
		return {
			...
			app:root.indexOf('?starter=') !== -1 ? root.replace(/[^\?]+\?(starter=)?/, '') : '' ,
	...
	// if first script loading
	if(!isIframe){
		var ch = checkBase() ;
		// create duplication iframe
		var loader = document.createElement('IFRAME') ;
		loader.setAttribute('id', 'strawloader') ;
		loader.setAttribute('name', 'strawloader') ;
		loader.setAttribute('class', 'abs left bottom') ;
		// loader.setAttribute('src', 'javascript:void(0)') ;
		...
		doc.open() ;
		doc.write('<body><scr'+'ipt type="text/javascript" src="'+ch.original+'"><\/scr'+'ipt><\/body>') ;
		doc.close() ;
	...
	}else{ // if we are in second script loading from iframe
		var PKG = {} , Type, Pkg ;
		
		Type = {
			globals:{},
			appdomain:isIframe ? window.top : window,
			guid:0,
		...
		// GLOBALS
		Type.appdomain.Type = Type ;
		Type.appdomain.Pkg = Pkg ;
		...
		Pkg.write('org.libspark.straw.core', function(){
			...
			var simfunc = function(resp, module, url){
				...
				return new Function('module', '__filename', '__dirname', '__parameters', 'with(window.top.Type.appdomain)with(module){' + resp + '};return module;')(module, filename, dirname, ModuleLoader.params) ;
			} ;
			...
			var app = pathes.app ;
			var root = pathes.root.replace('strawnode.js', '') ;
			...
			var ModuleLoader = Type.define(function(){
				...
				return {
					...
					load:function load(url, async){
						...
						r.open(ud['post_method'], loc+'?'+ uid , async) ;                             
			...
			var as_file = function as_file(filename){
						trace( 'end for now' , filename) ;
				var url = /[.]js/.test(url) ? filename  : filename + '.js';
			...
			// REQUIRE
			var require = Type.appdomain.require = function require(id){ // id is always string
				var s ;
				// cache checks
				if(!!(s = cache[id])) return s instanceof Module ? s.exports : s ;
				// if id is core module [ended inevitabely in window]
				if(!!(s = Type.appdomain[id])) return s instanceof Module ? s.exports : s ;
