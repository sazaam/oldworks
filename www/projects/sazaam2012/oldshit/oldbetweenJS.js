...
var BetweenJS = NS('BetweenJS', NS('org.libspark.betweenJS::BetweenJS', Class.$extend({
    __classvars__:{version:'0.65',
		...
		interval:function(closure, time)//:ITween
        {
            return new IntervalTween(closure, time) ;
        },
        timeout:function(closure, time)//:ITween
        {
            return new TimeoutTween(closure, time) ;
        },
...
var DOMElementUpdater = NS('DOMElementUpdater', NS('org.libspark.betweenJS.core.updaters::DOMElementUpdater', ObjectUpdater.$extend({
    __classvars__:{
        version:'0.65',
        register:function(registry){
            //registry.registerClassWithTargetClassAndPropertyName(ObjectUpdater, Object, '*');
    ...
    setSourceValue:function(propertyName, value, isRelative){
        var name = this.retrieveUnits(propertyName)
        if(isNaN(value)) value = this.target.style[name].replace(/px|em|pc|%/gi, '') ;
        this.$super(this.retrieveUnits(propertyName), value, isRelative) ;
    },
    setDestinationValue:function(propertyName, value, isRelative){
        this.$super(this.retrieveUnits(propertyName), value, isRelative) ;
    },
...
// TWEENS
var TimeoutTween = NS('TimeoutTween', NS('org.libspark.betweenJS.tweens::TimeoutTween', AbstractTween.$extend({
    __classvars__:{
        version:'0.65',
        toString:function()
        {
            return '[class TimeoutTween]' ;
        }
    },
    __init__:function(ticker)
    {
