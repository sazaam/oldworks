...
var Loader = (function(){
	...
	return function(url, complete, postData, async){
		var r = generateXHR() ;
		if (!r) return ;
		this.request = r ;
		this.url = url ;
...
var XApp = function(params){
	...
	XBase = app.base = new XUrl() ;
	XBase.go(root) ;
	
	XModulesBase = app.modulesbase = new XUrl(XBase.absoluteroot) ;
	...
	var appendScript = function(str, elName, isText){
	...
	for(var j = 0 ; j < len ; j++){
		...
		if(!!xloaded[mod.name]) continue ;
			...
			var lload = arr[j] = new Loader(url).load(undefined, false) ;
	...
	setTimeout(function(){
		appendScript(concatenated, 'head', true) ;
		concatenated = '' ;
	}, 0) ;
