	...
	Jade.prototype.parse = function parse(locals, text){
		...
		if(allincludes_r.test(all)){
			all = all.replace(allincludes_r, function parseForIncludes(){
				var ttt = arguments[1].replace(TAB, '') ;
				var path = arguments[3] ;
				if(!ext_r.test(path)){
					path = path + '.jade' ;
				}
				var msg = TAB + new Jade(path).load(undefined, false).response.replace(crlf_r, NLTAB) ;
