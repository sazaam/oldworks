...
/* NET */
Pkg.write('naja.net', function(path){
	...
	var Request = Type.define({
		...
		constructor:function Request(url, complete, postData) {
		...
		load:function(url){
			var r = this.request, th = this, ud = this.userData, complete = this.complete ;
	...
	var Address = Type.define({
		domain:Type.appdomain,
		constructor:function Address(str){
			var u = this ;
			u.absolute = str ;
...
/* EVENTS */
Pkg.write('naja.event', function(path){
		...
		copyFrom:function(source)
		{
			...
			if(listeners !== undefined){
				for (var type in listeners) {
					...
					var l = list.length;
					for (var i = 0; i < l; ++i) this.addEL(type, list[i].listener);
				}
			}
			return this ;
		}
	...
	return [IEvent, EventDispatcher] ;
}) ;

Pkg.write('naja.event::EventEnhancer', {
    domain:Type.appdomain,
	statics:{
		initEvents:function(){
			var ww = $(window) ;
    ...
    constructor:function EventEnhancer() {
...
/* COMMANDS */
Pkg.write('naja.command', function(path){
	
	var Command = Type.define({
		domain:Type.appdomain,
		inherits:EventDispatcher,
		constructor:function Command(thisObj, closure, params) {
		...
		constructor:function CommandQueue() {
			...
			this.add = function(){
				...
				switch(l)
				{
					...
					break;
					default :
						for(var i = 0 ; i < l ; i++ ) arguments.callee(args[i]) ;
					break;
				}
			}
	...
	var WaitCommand = Type.define({
		domain:Type.appdomain,
		inherits:Command,
		constructor:function WaitCommand(time, initclosure) {
	...
	var AjaxCommand = Type.define({
		domain:Type.appdomain,
		inherits:Command,
		constructor:function AjaxCommand(url, success, postData, init) {
	...
	return [Command, CommandQueue, WaitCommand, AjaxCommand] ;
}) ;

/* COLLECTION */
Pkg.write('naja.collection', function(path){
	
	var Loop = Type.define({
		domain:Type.appdomain,
		constructor:function Loop(){
	...
	var Cyclic = Type.define({
		domain:Type.appdomain,
		constructor:function Cyclic(arr){
			var cy = this ;
			cy.commands = [] ;
			cy.index = -1 ;
			cy.looping = true ;
			...
			if(!!arr && arr.length > 0) this.add.apply(this, arr) ;
		...
		add:function(){
			...
			for(var i = 0 ; i < len ; i++){
				var arg = args[i] ;
				if(!!arg[0] && arg[0] instanceof Command) l = cy.push.apply(cy, arg) ;
				else l = cy.push.apply(cy, [arg]) ;
			}
			return l ;
		},
		remove:function(){
			...
			for(var i = 0 ; i < len ; i++){
				var arg = args[i] ;
				if(isNaN(arg) && arg instanceof Command) cy.splice(cy.indexOf(arg), 1) ;
				else cy.splice(arg, 1) ;
			}
			var l = commands.length ;
			return l ;
		},
		indexOf:function(el){
			...
			if(Array.prototype['indexOf'] !== undefined)
				...
				for(var i = 0, l = commands.length ; i < l ; i++) if(commands[i] === el) return i ;
			return -1 ;
		},
		splice:function(){
			var cy = this ;
		...
		seek:function(rad){ // relative deg (degree) as relative position index in Array
			...
			if(rad > pi2 || pi2 - rad < cy.unit.rad / 2 ) rad = 0 ;
...
/* STEP */
Pkg.write('naja.step', function(path){
	...
	var HierarchyChanger = Type.define({
		...
		__futurePath:'',    
		constructor:function HierarchyChanger(){
			trace(this.__value) ;
		},
	...
	var Hierarchy = Type.define({
		...
		// CTOR
		constructor:function Hierarchy(){
			//
		},
	...
	var AddressHierarchy = Type.define({
		...
		constructor:function AddressHierarchy(s){
	...
	var AddressChanger = Type.define({
		...
		constructor:function AddressChanger(s){
	...
	return [Step, HierarchyChanger, Hierarchy, AddressHierarchy, AddressChanger] ;
}) ;

/* MAIN NAJA TOOLKIT */
Pkg.write('naja.toolkit::Toolkit', {
	domain:Type.appdomain,
	statics : {
        Qexists : function Qexists(sel) {
