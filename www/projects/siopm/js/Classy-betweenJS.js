 ...
 * 
 * Dependancies : 
 * 	jQuery 1.6.1+ (required for events handling)
 * 	Classy (Class inheritance pattern implementation) 
 * 	ns (packages for classy) 
 * 
 * @author : saz aka True
 * sazaam(at)gmail.com
 * 
...
if(NS('naja.net::IEvent') === undefined){ // Forcing Writing Naja evt implementation
	
	/* EVENTS jQueryfy (semi-dangerous) */
	for(var i in Class){
	   jQuery.Event.constructor.prototype[i] = Class[i] ;
	}

	var IEvent = NS('IEvent', NS('naja.net::IEvent', jQuery.Event.$extend({
	   __classvars__:{
		  version:'0.0.1',
		  toString:function()
		  {
	...
	var EventDispatcher = NS('EventDispatcher', NS('naja.event::EventDispatcher', Class.$extend({
    __classvars__:{
        version:'0.0.1',
        dependancies:[jQuery],
        toString:function()
    ...
    setDispatcher:function(el) // @void
    {
        this.dispatcher = $(el) ;
...
var BetweenJS = (function(){
	...
	*/
	...
	var CSSPropertyMapper = NS('CSSPropertyMapper', NS('org.libspark.betweenJS::CSSPropertyMapper', Class.$extend({
		__classvars__:{version:'0.95',
			toString:function(){
				return '[class CSSPropertyMapper]' ;
			},
			core:function(){
			...
			check:function(name){
				...
				switch(true){
					...
					case formats['separatorprop'].test(name) :
						return (cache[name] = arguments.callee(name.replace(/-(\w)/g, function($0, $1){return $1.toUpperCase()}))) ;
					break ;
					case formats['colorextprop'].test(name) :
						return (cache[name] = arguments.callee(name + '::#')) ;
					break ;
	...
	var UpdaterFactory = NS('UpdaterFactory', NS('org.libspark.betweenJS.core.updaters::UpdaterFactory', Class.$extend({
		...
		listPool:[],
		__init__:function(){
		   return (UpdaterFactory.instance = this) ;
		},
	...
	var BezierUpdater = NS('BezierUpdater', NS('org.libspark.betweenJS.core.updaters::BezierUpdater', ObjectUpdater.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
	...
	var PhysicalUpdater = NS('PhysicalUpdater', NS('org.libspark.betweenJS.core.updaters::PhysicalUpdater', ObjectUpdater.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
	...
	var AbstractActionTween = NS('AbstractActionTween', NS('org.libspark.betweenJS.core.tweens::AbstractActionTween', AbstractTween.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
				return '[class AbstractActionTween]' ;
			}
		},
		lastTime:undefined,
	...
	// ACTIONS
	var FunctionAction = NS('FunctionAction', NS('org.libspark.betweenJS.actions::FunctionAction', AbstractActionTween.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
				return '[class FunctionAction]' ;
			}
		},
		func:undefined,
		params:undefined,
	...
	var TimeoutAction = NS('TimeoutAction', NS('org.libspark.betweenJS.actions::TimeoutAction', AbstractActionTween.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
				return '[class TimeoutAction]' ;
			}
		},
		duration:0,
		func:undefined,
	...
	var IntervalAction = NS('IntervalAction', NS('org.libspark.betweenJS.actions::IntervalAction', AbstractActionTween.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
				return '[class IntervalAction]' ;
			}
		},
		duration:0,
		func:undefined,
	...
	var AddChildAction = NS('AddChildAction', NS('org.libspark.betweenJS.actions::AddChildAction', AbstractActionTween.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
				return '[class AddChildAction]' ;
			}
		},
		target:undefined,
		parent:undefined,
	...
	var RemoveFromParentAction = NS('RemoveFromParentAction', NS('org.libspark.betweenJS.actions::RemoveFromParentAction', AbstractActionTween.$extend({
		__classvars__:{
			version:'0.95',
			toString:function(){
				return '[class RemoveFromParentAction]' ;
			}
		},
		target:undefined,
