	...
	*/
		...
		__classvars__:{
			...
			cssSimpleGet:function(target, pname, unit){
				...
				if(str == '') str = CSSPropertyMapper.hasComputedStyle ? 
					window.getComputedStyle (target, '')[pname].replace(/(px|em|pc|pt|%)$/, '') :
					target.currentStyle[pname].replace(/(px|em|pc|pt|%)$/, '') ;
	...
	var AbstractTween = __global__.AbstractTween = Class('org.libspark.betweenJS.core.tweens::AbstractTween', {
		...
		bind:function(type, listener){
			return this.addEL(type, listener) ;
		},
		...
		unbind:function(type, listener){
			return this.removeEL(type, listener) ;
		},
		dispatch:function(e){
