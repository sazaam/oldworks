...
function setupTween()
{
	...
	*/
	...
	var twb = BetweenJS.delay(BetweenJS.to(current, {x: userOpts.range}, userOpts.duration*.001, Elastic.easeInOut), userOpts.delay * .001) ;
	var twback = BetweenJS.delay(BetweenJS.to(current, {x: -userOpts.range}, userOpts.duration*.001, Elastic.easeInOut), userOpts.delay* .001) ;
	TW = BetweenJS.serial(twb, twback) ;
	TW.onUpdate = update ;
	TW.stopOnComplete = false ;
	setTimeout(function(){TW.play()}, 0) ;
