var __dependancies__ = [
	...
	{name:'trace', url:'./trace.js'},
	{name:'classlite', url:'./classlite.js'},
...
ww.bind('load', function(e){
	...
	if(Toolkit.Qexists('.project')){
		...
		projects.each(function(i, el){
			...
			contblockinsides.each(function(k, lm){
				var infopage = $(lm) ;
		...
		nextproject.children('a').bind('mousedown', function(e){
			e.preventDefault() ;
			e.stopPropagation() ;
			all.data('projectswitch')() ;
		})

		prevproject.children('a').bind('mousedown', function(e){
			e.preventDefault() ;
			e.stopPropagation() ;
